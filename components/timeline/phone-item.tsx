  ...
  return (
    <Link href={`/sehir/${sehir.id}`}>
      <div className="w-full h-36 shadow rounded text-center relative hover:scale-105 duration-200 ease-in-out text-white">
        <Image src={sehir.thumbnail} alt="hakkari" fill className="object-cover" priority={true} quality={100} />
        <div className="absolute inset-0 bg-black/40" />
        <div className="absolute inset-0 w-full flex items-center justify-center font-semibold">{sehir.name}</div>
